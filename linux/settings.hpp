...
public:
    ...
    T get(QString keys) {
        ...
        Logger::print(QString("Found setting %1: %2 (default: %3) (settings: %4)").arg(keys).arg(loaded[keys.toStdString()].get<T>()).arg(isDefault ? "true" : "false").arg(this->loaded.dump()));
    ...
    bool set(T value, QString keys) {
        ...
        stream.close();
        Logger::print(QString("Found new setting %1: %2 (current: %3)").arg(keys).arg(loaded[keys.toStdString()].get<T>()).arg(loaded.dump()));
        return true;
    }
